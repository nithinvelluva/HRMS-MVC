...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        if (!this.options.preview) {
            ...
            $uploadBox = $('<div id="ssi-previewBox" class="ssi-uploadBox ssi-previewBox ' + (this.options.dropZone ? 'ssi-dropZonePreview ssi-dropZone' : '') + '"><div id="ssi-info">' + (this.options.dropZone ? '<div id="ssi-DropZoneBack">Drag and drop a file here or click</div>' : '') + '<div id="ssi-fileNumber" class="ssi-hidden">?</div></div></div>');
            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' +
                        '<tr class="disabled-element"><td class="ssi-upImgTd disabled-element">' + content + '</td></tr>' +
                        '<tr class="disabled-element"><td class="disabled-element"><div id="ssi-uploadProgress' + index + '" class="ssi-hidden ssi-uploadProgress"></div></td></tr>' +
                        '<tr><td><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"><span class="trash10 trash"></span></button></td></tr>' +
                        '<tr class="disabled-element"><td class="disabled-element">' + cutFileName(filename, ext, 15) + '</td></tr></table>'
                };
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        if (this.pending > 0) {
            ...
            files_to_upload = [];
            files_to_upload = thisS.toUpload;            
            //formData.append(thisS.inputName, thisS.toUpload[i]);//append the first file to the form data
            formData.append('CalendarEventFile', thisS.toUpload[i]);
            formData.append('event_id', $('#calendar_event_id').val());
